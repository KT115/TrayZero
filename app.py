...
# ==============================================================================
# 2. 雙 Hugging Face Pipeline 初始化與快取 (Cached Models)
# ==============================================================================
@st.cache_resource(show_spinner=False)
def load_hf_pipelines():
    # Pipeline 1: Object Detection (優先使用本地微調權重，否則使用官方 YOLOS-tiny)
    local_detector_path = "./Fine-tuned_Model_files"
    if os.path.exists(local_detector_path) and any(os.scandir(local_detector_path)):
        detection_model = local_detector_path
    else:
        detection_model = "hustvl/yolos-tiny"
    
    device = 0 if torch.cuda.is_available() else -1
    
    detector = pipeline("object-detection", model=detection_model, device=device)
    generator = pipeline("text2text-generation", model="google/flan-t5-base", device=device)
    
    return detector, generator

with st.spinner("🚀 正在初始化雙核心 Hugging Face 深度學習模型 (YOLOS-tiny + Flan-T5)..."):
    detector_pipeline, generator_pipeline = load_hf_pipelines()
...
def run_detection_and_draw(image, threshold=0.25):
    # 執行 Pipeline 1
    raw_results = detector_pipeline(image)
    ...
    for res in raw_results:
        ...
        # 換算食物殘渣模擬標籤 (將通用目標映射至快餐殘留實體)
        mapped_label = label
        if label in ["bowl", "dining table"]:
            mapped_label = "Rice / Base Waste (白飯殘留)"
        elif label in ["sandwich", "pizza"]:
            mapped_label = "Meat / Protein Residual (主菜肉類殘留)"
        else:
            mapped_label = f"Residue: {label} (醬汁/配菜)"
            
        xmin, ymin, xmax, ymax = box["xmin"], box["ymin"], box["xmax"], box["ymax"]
        box_area = (xmax - xmin) * (ymax - ymin)
        waste_area_covered += box_area
        
        box_color = color_map.get(label, "#FF0000")
    ...
    estimated_waste_ratio = min(1.0, waste_area_covered / (total_tray_area * 0.7))
...
with tab_audit:
    ...
    with col_input:
        ...
        if input_type == "自訂上傳餐盤相片":
            ...
            # 建立多物件視覺模擬樣本，供 Object Detection 畫框
            mock_img = Image.new("RGB", (400, 300), color=(225, 220, 210))
            d = ImageDraw.Draw(mock_img)
            d.rectangle([30, 30, 370, 270], fill=(200, 195, 185), outline=(100, 100, 100), width=3) # 餐盤
            
            if "樣本 A" in sample_name:
                d.ellipse([70, 60, 220, 210], fill=(255, 255, 250), outline=(180, 180, 170), width=2) # 碗/飯
                d.rectangle([240, 80, 330, 180], fill=(130, 70, 40)) # 肉排殘留
            elif "樣本 B" in sample_name:
                d.ellipse([140, 80, 260, 200], fill=(180, 180, 170), outline=(120, 120, 120), width=2) # 空碗
            else:
                d.rectangle([90, 80, 210, 220], fill=(140, 60, 30)) # 大塊肉排剩餘
                d.ellipse([230, 90, 320, 180], fill=(60, 120, 50)) # 蔬菜剩餘
            raw_img = mock_img
    ...
    with col_result:
        ...
        if raw_img and st.button("🚀 啟動雙 Pipeline 深度偵測與審計", type="primary"):
            # Step A: 執行 Pipeline 1 (Object Detection)
            with st.spinner("Pipeline 1 運算中: YOLOS 正在偵測邊界框與殘食佔比..."):
                annotated_img, summary_data, waste_ratio = run_detection_and_draw(enhanced_img)
            ...
            # Step B: 執行 Pipeline 2 (Context-Aware Flan-T5)
            # 依據分店等級注入客製化策略
            prompt_text = (
            ...
            with st.spinner("Pipeline 2 運算中: Flan-T5 正在生成該分店之營運指令..."):
                nlp_output = generator_pipeline(prompt_text, max_length=120, do_sample=False)
                kitchen_directive = nlp_output[0]["generated_text"]
